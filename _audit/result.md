...
图片留白修复：主要空白来自固定 4:3 容器，现改为等宽、自然高度；桌面图片列由 175px 调整为 210px，手机预览宽度最多 360px，不再限制图片高度。五张原图裁去外围近白边缘、保留 4px 安全边距；不裁内部模块、不拉伸。裁剪入口为 crop-previews.cjs，使用环境已有 Sharp，不增加站点依赖。新增浏览器检查比较图片内容区域与自然宽高比，旧样式下失败；原图可从 Git 历史恢复。
